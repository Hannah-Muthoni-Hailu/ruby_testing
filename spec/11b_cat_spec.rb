...
describe Cat do
  ...
  subject(:wasabi) { described_class.new('Wasabi') }

  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'when both Cat and Dog have the talk function' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not hungry' do
    expect(wasabi).not_to be_hungry
  end

  # remove the 'x' before running this test
  it 'is hiding' do
    expect(wasabi).to be_hiding
  end
end
