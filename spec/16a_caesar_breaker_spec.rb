...
describe CaesarBreaker do
  ...
  describe '#save_to_yaml' do
    ...
    it 'dumps to yaml' do
      m = phrase.instance_variable_get(:@message)
      d = phrase.instance_variable_get(:@decrypted_messages)
      expect(YAML).to receive(:dump).with('message' => m, 'decrypted_messages' => d).once
      phrase.save_to_yaml
    end
  end
end
