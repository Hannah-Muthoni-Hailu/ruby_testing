...
describe FindNumber do
  ...
  describe '#game_over?' do
    ...
    let(:number_two) { double('random_number', value: 3) }
    context 'when guess and random_number are equal' do
      ...
      subject(:game) { described_class.new(0, 9, number_two, 3) }
    ...
    context 'when guess and random_number are not equal' do
      subject(:game_one) { described_class.new(0, 9, number_two, 2) }
      it 'is not game over' do
        expect(game_one).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do # I should have used before {} here
      subject(:mid_guess_game) { described_class.new(5, 8, number_range, 7) }
      it 'updates min to the same value as max' do
        mid_guess_game.update_range
        expect(mid_guess_game.max).to eq(mid_guess_game.min)
      end

      it 'does not update max' do
        mid_guess_game.update_range
        expect(mid_guess_game.max).to eq(8)
      end
    end
  end
end
